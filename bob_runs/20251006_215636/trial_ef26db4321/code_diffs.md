...
+    try:
+        risk = abs(entry - sl)
+        rr_tp = entry + 2.287314825669556 * risk if side == 'BUY' else entry - 2.287314825669556 * risk
+    except Exception:
+        risk = 0.0
...
+        imp_tp = rr_tp
+    try:
+        tp = 0.43020825806551516 * rr_tp + 0.1682812989386039 * band_tp + 0.19555289626322392 * imp_tp
+        if risk > 0:
+            if side == 'BUY':
+                tp = max(tp, entry + 2.287314825669556 * risk)
+            else:
+                tp = min(tp, entry - 2.287314825669556 * risk)
+        tp_hint = tp
+    except Exception:
+        pass
```
