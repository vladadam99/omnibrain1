...
+    try:
+        risk = abs(entry - sl)
+        rr_tp = entry + 1.957788649848505 * risk if side == 'BUY' else entry - 1.957788649848505 * risk
+    except Exception:
+        risk = 0.0
...
+        imp_tp = rr_tp
+    try:
+        tp = 0.6504024929716363 * rr_tp + 0.4164322807531513 * band_tp + 0.4821186613638078 * imp_tp
+        if risk > 0:
+            if side == 'BUY':
+                tp = max(tp, entry + 1.957788649848505 * risk)
+            else:
+                tp = min(tp, entry - 1.957788649848505 * risk)
+        tp_hint = tp
+    except Exception:
+        pass
```
