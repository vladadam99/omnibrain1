...
# Optional fallback: pull token/chat from REALBOB.json if env is empty
try:
    if not _TG_TOKEN or not _TG_CHAT:
        with open("REALBOB.json","r",encoding="utf-8") as _f:
            _cfg = _json.load(_f)
        _TG_TOKEN = _TG_TOKEN or _cfg.get("TELEGRAM_TOKEN") or _cfg.get("token") or ""
        _TG_CHAT  = _TG_CHAT  or str(_cfg.get("TELEGRAM_CHAT_ID") or _cfg.get("chat_id") or "")
except Exception:
    pass
...
def _tg_progress(done, total, best):
    ...
    try:
        tok, chat = _tg_creds()
        if pct % 10 == 0 and tok and chat and pct not in _TG_SENT:
            _TG_SENT.add(pct)
            best_id = None; best_score = None; pnl = None; wr = None
            try:
                best_id = best.get('trial_id')
                best_score = best.get('score')
                m = best.get('metrics', {}) or {}
                pnl = m.get('pnl_total'); wr = m.get('winrate')
            except Exception:
                pass
            msg = f"? Bob progress: {pct}% ({done}/{total})"
            if best_id is not None and best_score is not None:
                msg += f" | best={best_id} score={best_score:.4f}"
            if pnl is not None and wr is not None:
                msg += f" pnl={pnl:.2f} wr={wr:.3f}"
    ...
    try:
        ...
        if tok and chat:
            try:
                ...
                sym_preview = ""
            send_telegram_message(tok, chat, f"?? Bob started | run={run_id} | tf={cfg.get('tf')} | symbols={sym_preview}")
    except Exception:
        pass
